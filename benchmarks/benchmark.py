...
def run_benchmark(shape=(100, 100, 100), steps=50):
    Nx, Ny, Nz = shape
    ...
    np_mon = NumPyNear2FarMonitor(np_sim, ctr_phys, size_phys, freq)

    # Run and time
    start_time = time.perf_counter()
    np_sim.run(steps, progress_every=10)
    ...
    # GPU-accelerated monitor
    cl_mon = OpenCLNear2FarMonitor(cl_sim, ctr_phys, size_phys, freq)

    # Run and time
    start_time = time.perf_counter()
    cl_sim.run(steps, progress_every=10)
    # Complete queue execution before measuring end time
    cl_sim.queue.finish()
    ...
    print("=" * 60)
    print(f"Grid size: {total_cells/1e6:.2f}M cells, {steps} steps")
